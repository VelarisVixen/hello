...
export const handleAssess: RequestHandler = async (req, res) => {
  ...
  const { symptoms, age, sex } = parse.data;
  ...
  const prompt = `You are a cautious medical assistant. Based on the user's symptoms, provide: 1) top 3-5 possible causes (lay language), 2) red-flag warnings that require urgent care, 3) suggested next steps and self-care, 4) what to tell a clinician. Always include a disclaimer that this is not a diagnosis.\n\nUser info: ${age?`Age: ${age}. `:""}${sex?`Sex: ${sex}. `:""}Symptoms: ${symptoms}`;
