...
export const handleAssess: RequestHandler = async (req, res) => {
  ...
  if (!apiKey) {
    return res
      ...
      .json({
        error:
          "GEMINI_API_KEY is not configured. Set it in environment variables and restart the server.",
      });
  }
  ...
  try {
    ...
    if (!resp.ok) {
      return res
        .status(500)
        .json({ error: json.error?.message || "Gemini error" });
    }
    ...
    if (parsed) {
      return res.json({
        ...
        disclaimer:
          parsed.disclaimer ??
          "This is not a diagnosis. Consult a healthcare professional.",
      });
    }
    return res.json({
      analysis: text,
